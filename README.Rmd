...
We'll simulate data to run the ICED model on. The `sim_ICED` function takes the model structure dataframe we used earlier and a list of variances for each latent variable.
...
we can now run our model. The `run_ICED` function will print a bunch of relevant outputs
